...
export function formatUniqueNames(names: string[]): string[] {
    return names.map(formatUniqueName);
}

export function formatUniqueName(name: string): string {
    ...
    if (words.length === 1) {
        const fw = words[0][0].toLowerCase() + words[0].substr(1);
        // Name->name === name
        if (fw === words[0].toLowerCase()) {
...
function removeSymbolsFromText(text) {
    ...
    // var reg = /[=~`!@#$%^&*\(\)_<>,:'";\[\]{}|?/\-]+/g;
    return text.replace(reg, ' ').replace(/ {2,}/g, ' ').trim();
}
